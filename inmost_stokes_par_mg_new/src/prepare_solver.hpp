...
#include <string>
#include <iostream>
#include "prepare_preconditioner.hpp"
...
auto prepare_solver_block_SPAI(std::map< std::string, Mat > &submatrices, 
    std::map< std::string, IS > &index_sequences) {
    std::cout << "prepare_solver_block_SPAI\n";
    ...
    auto [ctx_full] = prepare_preconditioner_context( submatrices, index_sequences );
    ...
    KSPSetConvergenceTest(ksp, convergence_test_full, ctx_full, ConvergenceDestroy);
