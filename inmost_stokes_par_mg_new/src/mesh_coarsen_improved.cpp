                           ...
                           int MISKDistance)
{
    ...
    std::unordered_map< int, std::vector< int > > gid_to_get_from_rank;
    ...
    for(int i = 0 ; i < cell_center_coordinates.size() ; i++) {
        // std::cout << "rank = " << rank 
        //           << " gid = " << i + A_lapl_N1 
        //           << " coord = " << std::get<0>(cell_center_coordinates[i])
        //           << " " << std::get<1>(cell_center_coordinates[i])
        //           << " " << std::get<2>(cell_center_coordinates[i]) << "\n";
    }

    MPI_Comm send_size_comm;
    ...
    std::vector<int> gid_sizes_to_receive;
    ...
    std::vector<int> gid_list_to_send;
    ...
    std::vector< std::tuple< double, double, double > > coord_to_receive;
    ...
    //     bool containsDuplicates = (std::unique(gid_list.begin(), gid_list.end()) != gid_list.end());
    ...
    //     bool containsDuplicates = (std::unique(coarse_local_index.begin(), coarse_local_index.end()) != coarse_local_index.end());
    //     std::cout << containsDuplicates << "\n";
    //     std::exit(0);
    // }
    ...
    for( int i = 0 ; i < gid_list.size() ; i++ ) {
        ...
        coarse_local_index.push_back( gid - A_lapl_N1 + face_unknowns_local );
        all_index[ gid - A_lapl_N1 + face_unknowns_local ] = 1;
    }
    ...
    return std::make_tuple( coarse_local_index, fine_local_index, coarse_cell_center_coordinates, coarse_cell_to_face_local, coarse_face_unknowns_local );
}